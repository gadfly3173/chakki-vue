      ...
      <div class="wrapper">
        ...
        <el-table :data="announceList" style="width: 100%">
          ...
          <el-table-column label="操作" fixed="right" width="200">
            <template slot-scope="scope">
              <el-button @click.stop="handleViewStudentClick(scope.row.id)" type="primary" plain size="mini">
                查看
              </el-button>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    async getAnnounceList() {
      try {
        this.loading = true
        const res = await Class.getStudentAnnounceList(this.currentClassId, this.pageSize, this.currentPage - 1)
    ...
    handleViewStudentClick(id) {
      this.$router.push({ path: `/class/room/announce/view/${id}` })
    },
